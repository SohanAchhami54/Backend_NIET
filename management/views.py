...
#         batch = Batch.objects.filter(is_active=True)
...
# class BatchSemesterDetail(APIView):
#     def get(self,request):
#         batch_semester = BatchSemester.objects.filter(is_running=True, is_active=True)
#         serializer = BatchSemesterDetailSerializer(batch_semester,many=True)
#         return Response(serializer.data,status=status.HTTP_200_OK)

# class BatchSemesterHistory(APIView):
#     def get(self,request,year):
#         batch_semester = BatchSemester.objects.filter(batch=Batch.objects.get(year=year),is_running=False, is_active=True)
#         serializer = BatchSemesterDetailSerializer(batch_semester,many=True)
#         return Response(serializer.data,status=status.HTTP_200_OK)

# class BatchSemesterFilter(APIView):
#     def get(self,request,id):
#         batch_semester = BatchSemester.objects.filter(batch=Batch.objects.get(id=id))
#         serializer = BatchSemesterDetailSerializer(batch_semester,many=True)
#         return Response(serializer.data,status=status.HTTP_200_OK)

# class StudentBatchSemesterList(APIView):
#     def get(self,requests,batch_id,semester_id):
#         try:
#             batch_semester = BatchSemester.objects.get(
#                 batch = Batch.objects.get(id=batch_id),
#                 semester = Semester.objects.get(id=semester_id)
#                 )
#             student_in_sem = StudentInSemester.objects.filter(semester=batch_semester)
#         except Exception as e:
